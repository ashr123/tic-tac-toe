package io.github.ashr123.tictactoe.controller;

import io.github.ashr123.tictactoe.enumeration.GameState;
import io.github.ashr123.tictactoe.manager.TicTacToeManager;
import io.github.ashr123.tictactoe.model.TicTacToe;
import io.github.ashr123.tictactoe.model.dto.JoinMessage;
import io.github.ashr123.tictactoe.model.dto.PlayerMessage;
import io.github.ashr123.tictactoe.model.dto.TicTacToeMessage;
...
public class MessageController {
    ...
    public Object joinGame(@Payload JoinMessage message, SimpMessageHeaderAccessor headerAccessor) {
        TicTacToe game = ticTacToeManager.joinGame(message.player());
        if (game == null) {
            TicTacToeMessage errorMessage = new TicTacToeMessage();
            errorMessage.setType("error");
            errorMessage.setContent("Unable to enter the game. The game may already be full or an internal error occurred.");
            return errorMessage;
        }
        headerAccessor.getSessionAttributes().put("gameId", game.getGameId());
        headerAccessor.getSessionAttributes().put("player", message.player());
    ...
    public void leaveGame(@Payload PlayerMessage message) {
        TicTacToe game = ticTacToeManager.leaveGame(message.player());
        if (game != null) {
            TicTacToeMessage gameMessage = gameToMessage(game);
    ...
    private static TicTacToeMessage gameToMessage(TicTacToe game) {
