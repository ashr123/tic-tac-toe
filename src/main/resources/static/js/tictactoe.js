 ...
 */
const sendMessage = (message) => stompClient.send(`/app/${message.type}`, {}, JSON.stringify(message))
 ...
 */
const messagesTypes = {
	"game.join": (message) => updateGame(message),
	...
	"game.joined": (message) => {
		...
		stompClient.connect(
			{},
			frame => stompClient.subscribe(
				`/topic/game.${message.gameId}`,
				message => handleMessage(JSON.parse(message.body))
			)
		);
	},
	"game.move": (message) => updateGame(message),
	"game.left": (message) => {
		updateGame(message);
		if (message.winner)
			showWinner(message.winner);
	},
	"error": (message) => toastr.error(message.content)
}
...
const connect = () => {
	...
	stompClient.connect({}, frame => {
		stompClient.subscribe('/topic/game.state', message => handleMessage(JSON.parse(message.body)));
		loadGame();
	});
}
